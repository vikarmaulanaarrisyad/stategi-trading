...
def print_banner(cfg: TradingConfig, broker_symbol: str, account_info):
    ...
    print(f" Filter SNR       : {'Aktif (Obstacle & Confluence)' if getattr(cfg, 'use_snr_filter', True) else 'Nonaktif'}")
    print(f" Filter SMC       : {'Aktif (Discount/Prem & Sweep)' if getattr(cfg, 'use_smc_filter', True) else 'Nonaktif'}")

...
class SupportResistanceAnalyzer:
    ...
    def analyze_snr(self, df: pd.DataFrame, shift: int = -1) -> SnrSnapshot:
        ...
        if shift < 25 or shift >= n:
            return SnrSnapshot(None, None, None, None, False, False, "Data tidak cukup")
        ...
        # 2. Ambil Major Support & Resistance Terdekat
        resistances = [h[1] for h in (major_highs + minor_highs) if h[1] > (current_close + zone_buffer * 0.5)]
        supports = [l[1] for l in (major_lows + minor_lows) if l[1] < (current_close - zone_buffer * 0.5)]
        ...
        # Swing high lama yang sudah di-breakout oleh close price, dan sekarang berada di bawah harga
        nearest_rbs = None
        for bar_idx, p_high in (major_highs + minor_highs):
            # Syarat: harga pernah menembus ke atas level ini setelah terbentuk
            if p_high < current_close:
                # Cek apakah pernah ada candle yang close tegas di atas level ini
        ...
        # Swing low lama yang sudah di-breakdown oleh close price, dan sekarang berada di atas harga
        nearest_sbr = None
        ...
        for sup in key_supports:
            # Low candle menyentuh area zona support (antara sup - buffer hingga sup + buffer)
    ...
    def is_obstacle_blocking(self,
        ...
        """
        Memeriksa apakah ada level S/R Mayor yang menghalangi jalan menuju target keuntungan.
        Jika jarak ke S/R rintangan lebih kecil dari (sl_dist * min_clearance_rr), order ditolak.
        """
        required_clearance_dist = (sl_dist_points * point_size) * min_clearance_rr

        if signal_type == "BUY":
            res = snapshot.nearest_resistance

//+------------------------------------------------------------------+
//|                                     DoublePattern_RiskManager.mqh|
//|                 Risk Management Module for Double Top & Bottom   |
...
//+------------------------------------------------------------------+
//| Class Manajer Resiko Terpadu Double Top & Bottom                 |
//+------------------------------------------------------------------+
class CDoublePatternRiskManager
  {
private:
   int               m_dailyLossCount;        // Jumlah trade kalah hari ini
   double            m_dailyLossAmount;       // Nominal total loss hari ini ($)
   int               m_lastCalculatedDay;     // Hari kalender perhitungan terakhir

public:
                     CDoublePatternRiskManager();
                    ~CDoublePatternRiskManager();
   ...
   // Proteksi Akhir Pekan (Friday Auto-Close)
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CDoublePatternRiskManager::CDoublePatternRiskManager()
  {
   m_dailyLossCount    = 0;
...
//| Hitung Lot Size Dinamis Berdasarkan % Resiko Modal               |
//+------------------------------------------------------------------+
double CDoublePatternRiskManager::CalculateLotSize(double equity, double riskPercent, double slDistancePoints,
                                                   string symbol, double fixedLot = 0.01, bool useFixedLot = false,
                                                   double maxLotSize = 5.0)
  {
...
//+------------------------------------------------------------------+
void CDoublePatternRiskManager::UpdateDailyStats(ulong magic, string symbol)
  {
   MqlDateTime dt;
   TimeCurrent(dt);
...
//+------------------------------------------------------------------+
bool CDoublePatternRiskManager::IsDailyLossLimitReached(ulong magic, string symbol, double equity,
                                                        bool useDailyLimit, int maxLosses, double maxLossPercent)
  {
   ...
   if(m_dailyLossCount >= maxLosses)
     {
      Print(StringFormat("[Pattern Circuit Breaker] Batas kalah harian (%d trade) tercapai. Trading ditutup hingga besok.",
                         m_dailyLossCount));
      return true;
     }
   ...
   if(m_dailyLossAmount >= maxAllowedLoss && maxAllowedLoss > 0.0)
     {
      Print(StringFormat("[Pattern Circuit Breaker] Batas nominal loss harian ($%.2f / $%.2f) tercapai. Trading ditutup hingga besok.",
...
//+------------------------------------------------------------------+
bool CDoublePatternRiskManager::IsFridayTradingRestricted(bool useFridayClose, int closeHour, int closeMinute,
                                                          bool blockNewTrades, int stopTradeHour)
  {
...
//+------------------------------------------------------------------+
void CDoublePatternRiskManager::CheckFridayAutoClose(CTrade &tradeObj, CPositionInfo &posInfo, ulong magic, string symbol,
                                                     bool useFridayClose, int closeHour, int closeMinute)
  {
         ...
         for(int i = PositionsTotal() - 1; i >= 0; i--)
           {
            ...
            if(tradeObj.PositionClose(ticket))
              {
               Print(StringFormat("[Pattern Friday Auto-Close] Posisi #%I64u ditutup sebelum akhir pekan.", ticket));
              }
            else
              {
               Print(StringFormat("[Pattern Friday Auto-Close Error] Gagal menutup posisi #%I64u: %s", ticket, tradeObj.ResultRetcodeDescription()));
              }
           }
        }
     }

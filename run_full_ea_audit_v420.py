...
def run_backtest_audit(rates_m5, rates_m15, strict_trend=True, pro_pullback=True,
    ...
    for i in range(150, total_bars):
        ...
        ema_slope = (ema125_m5[i - 1] - ema125_m5[i - 6]) / pip_value if i >= 6 else 0.0
        ...
        if active_trade is not None:
            ...
            if t['type'] == 'BUY':
                ...
                if profit_pips >= 5.0 and c_close < ema21_m5[i] and closes[i - 1] >= ema21_m5[i - 1]:
                    ...
                    balance += pnl
                    trades.append({'time': times[i], 'type': 'BUY', 'pnl': pnl, 'reason': 'AUTO-CUT REVERSAL', 'bal': balance, 'pattern': t['pattern']})
                ...
                if c_low <= t['sl']:
                    ...
                    trades.append({'time': times[i], 'type': 'BUY', 'pnl': pnl, 'reason': reason, 'bal': balance, 'pattern': t['pattern']})
                    monthly_data[month_key]['trades'] += 1; monthly_data[month_key]['pnl'] += pnl
                    if pnl > 0: monthly_data[month_key]['wins'] += 1
                    else: monthly_data[month_key]['losses'] += 1
                    active_trade = None; continue
                ...
                if c_high >= t['tp']:
                    ...
                    balance += pnl
                    trades.append({'time': times[i], 'type': 'BUY', 'pnl': pnl, 'reason': 'TAKE PROFIT', 'bal': balance, 'pattern': t['pattern']})
                    monthly_data[month_key]['trades'] += 1; monthly_data[month_key]['pnl'] += pnl
                    if pnl > 0: monthly_data[month_key]['wins'] += 1
                    else: monthly_data[month_key]['losses'] += 1
                    active_trade = None; continue
                ...
                if profit_pips >= 5.0 and c_close > ema21_m5[i] and closes[i - 1] <= ema21_m5[i - 1]:
                    ...
                    balance += pnl
                    trades.append({'time': times[i], 'type': 'SELL', 'pnl': pnl, 'reason': 'AUTO-CUT REVERSAL', 'bal': balance, 'pattern': t['pattern']})
                    monthly_data[month_key]['trades'] += 1; monthly_data[month_key]['pnl'] += pnl
                    if pnl > 0: monthly_data[month_key]['wins'] += 1
                    else: monthly_data[month_key]['losses'] += 1
                    active_trade = None; continue
                ...
                if c_high >= t['sl']:
                    ...
                    balance += pnl
                    reason = 'SL+ LOCK WIN' if t['sl'] < t['open_price'] else 'STOP LOSS'
                    trades.append({'time': times[i], 'type': 'SELL', 'pnl': pnl, 'reason': reason, 'bal': balance, 'pattern': t['pattern']})
                    monthly_data[month_key]['trades'] += 1; monthly_data[month_key]['pnl'] += pnl
                    if pnl > 0: monthly_data[month_key]['wins'] += 1
                    else: monthly_data[month_key]['losses'] += 1
                    active_trade = None; continue
                ...
                if c_low <= t['tp']:
                    ...
                    balance += pnl
                    trades.append({'time': times[i], 'type': 'SELL', 'pnl': pnl, 'reason': 'TAKE PROFIT', 'bal': balance, 'pattern': t['pattern']})
                    monthly_data[month_key]['trades'] += 1; monthly_data[month_key]['pnl'] += pnl
                    if pnl > 0: monthly_data[month_key]['wins'] += 1
                    else: monthly_data[month_key]['losses'] += 1
                    active_trade = None; continue
        ...
        if strict_trend and dom_trend == "BEARISH":
            # Blokir buy saat tren makro bearish, KECUALI jika itu adalah liquidity sweep reversal (SSL Trap)
            if not is_bear_trap:
                buy_allowed = False
        ...
        if buy_allowed and (buy_trend_125 or is_bear_trap):
            if body_pct <= 22.0 and not is_bear_trap: pass
                ...
                if is_engulf and ((closes[i - 1] - ema21_m5[i - 1]) / pip_value) >= (1.5 * current_atr / pip_value): pass
                    ...
                    is_pro_pb = pro_pullback and is_pullback and in_gp and is_vsa_dry and candle_ok and (dom_trend == "BULLISH")
                    ...
                    if (is_pullback and candle_ok and score >= 65.0) or is_bear_trap or is_pro_pb:
                        ...
                        pat_label = "PRO-PULLBACK" if is_pro_pb else ("TRAP-HUNTER" if is_bear_trap else "PULLBACK-4PILLAR")
                        pattern_breakdown[pat_label] = pattern_breakdown.get(pat_label, 0) + 1
                        
                        active_trade = {'type': 'BUY', 'open_price': open_p, 'sl': sl_p, 'tp': tp_p, 'is_be': False, 'pattern': pat_label}
        ...
        if sell_allowed and (sell_trend_125 or is_bull_trap):
            if body_pct <= 22.0 and not is_bull_trap: pass
                ...
                if is_engulf and ((ema21_m5[i - 1] - closes[i - 1]) / pip_value) >= (1.5 * current_atr / pip_value): pass
                    ...
                    is_pro_pb = pro_pullback and is_pullback and in_gp and is_vsa_dry and candle_ok and (dom_trend == "BEARISH")
                    ...
                    if (is_pullback and candle_ok and score >= 65.0) or is_bull_trap or is_pro_pb:
                        ...
                        pat_label = "PRO-PULLBACK" if is_pro_pb else ("TRAP-HUNTER" if is_bull_trap else "PULLBACK-4PILLAR")
                        pattern_breakdown[pat_label] = pattern_breakdown.get(pat_label, 0) + 1

                        active_trade = {'type': 'SELL', 'open_price': open_p, 'sl': sl_p, 'tp': tp_p, 'is_be': False, 'pattern': pat_label}
                        last_order_bar = i; continue
...
def main():
    ...
    print("=" * 100)
    print("AUDIT & BACKTEST KOMPARATIF: BASELINE (v4.10) vs PRO TREND PULLBACK SCALPER (v4.20)")
    print(f"Asset: {symbol} | Total Bar M5: {len(rates_m5):,} | Periode: Jan 2026 - Sep 2026")
    print("Modal Awal: $20.00 | Lot: 0.01 Fixed")
    print("=" * 100)

    print("\n[1/2] Menjalankan Simulasi Baseline EA (v4.10 Tanpa Filter Tren Ketat)...")
    res_base = run_backtest_audit(rates_m5, rates_m15, strict_trend=False, pro_pullback=False)

    print("[2/2] Menjalankan Simulasi EA v4.20 (Pro Trend Pullback Scalper + Strict Bias)...")
    res_v420 = run_backtest_audit(rates_m5, rates_m15, strict_trend=True, pro_pullback=True)

    print("\n" + "=" * 100)
    print(f"{'METRIK PERFORMA':<32} | {'BASELINE (v4.10)':<25} | {'PRO PULLBACK (v4.20)':<25} | {'PENINGKATAN':<15}")
    print("-" * 100)
    print(f"{'Modal Awal':<32} | ${res_base['initial_balance']:<24.2f} | ${res_v420['initial_balance']:<24.2f} | -")
    print(f"{'Saldo Akhir (Final Balance)':<32} | ${res_base['final_balance']:<24.2f} | ${res_v420['final_balance']:<24.2f} | +${res_v420['final_balance'] - res_base['final_balance']:.2f}")
    print(f"{'Profit Bersih (Net Profit)':<32} | ${res_base['net_profit']:<24.2f} | ${res_v420['net_profit']:<24.2f} | +${res_v420['net_profit'] - res_base['net_profit']:.2f}")
    print(f"{'Return On Investment (ROI)':<32} | +{res_base['return_pct']:<23.1f}% | +{res_v420['return_pct']:<23.1f}% | +{res_v420['return_pct'] - res_base['return_pct']:.1f}%")
    print(f"{'Total Trades':<32} | {res_base['total_trades']:<25} | {res_v420['total_trades']:<25} | {res_v420['total_trades'] - res_base['total_trades']:+d}")
    print(f"{'Trades Menang / Kalah':<32} | {res_base['wins']} W / {res_base['losses']} L{'':<14} | {res_v420['wins']} W / {res_v420['losses']} L{'':<14} | -")
    print(f"{'Win Rate':<32} | {res_base['win_rate']:<24.2f}% | {res_v420['win_rate']:<24.2f}% | +{res_v420['win_rate'] - res_base['win_rate']:.2f}%")
    print(f"{'Profit Factor':<32} | {res_base['profit_factor']:<25.2f} | {res_v420['profit_factor']:<25.2f} | +{res_v420['profit_factor'] - res_base['profit_factor']:.2f}")
    print(f"{'Maksimum Drawdown ($)':<32} | ${res_base['max_dd_dollars']:<24.2f} | ${res_v420['max_dd_dollars']:<24.2f} | ${res_v420['max_dd_dollars'] - res_base['max_dd_dollars']:+.2f}")
    print(f"{'Maksimum Drawdown (%)':<32} | {res_base['max_dd_pct']:<24.2f}% | {res_v420['max_dd_pct']:<24.2f}% | {res_v420['max_dd_pct'] - res_base['max_dd_pct']:+.2f}%")
    print(f"{'Sinyal Kontra-Tren Dicegah':<32} | {'0 (Semua diambil)':<25} | {res_v420['blocked_counter_trend']:<25} | +{res_v420['blocked_counter_trend']} filter")
    print("=" * 100)

    print("\nDISTRIBUSI POLA EKSEKUSI v4.20:")
    for pat, count in res_v420['pattern_breakdown'].items():
        pct = (count / res_v420['total_trades']) * 100.0
        print(f"   • {pat:<20}: {count:>5} trades ({pct:>5.1f}%)")

    print("\nBREAKDOWN KINERJA BULANAN v4.20 (2026):")
    print("   Bulan      | Trades   | Menang | Kalah | Win Rate   | Profit Bersih ($)  | Status Kinerja")
    print("   " + "-" * 80)
    for m in sorted(res_v420['monthly_data'].keys()):
        d = res_v420['monthly_data'][m]
        wr = (d['wins'] / d['trades'] * 100.0) if d['trades'] > 0 else 0.0
        status = "PROFIT HIJAU (KONSISTEN)" if d['pnl'] > 0 else "LOSS"
        print(f"   {m:<10} | {d['trades']:<8} | {d['wins']:<6} | {d['losses']:<5} | {wr:>6.1f}%    | $ {d['pnl']:>14.2f}  | {status}")
    print("=" * 100)

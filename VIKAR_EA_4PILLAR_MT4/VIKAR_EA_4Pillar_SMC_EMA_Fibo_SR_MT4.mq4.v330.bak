...
#property description "3. Prop Firm Equity Guardian, Structural Trailing & Turtle Soup Trap Hunter"
...
extern ENUM_TP_TYPE  InpTPType             = TP_TYPE_RISK_REWARD;   // Metode Penentuan Target Take Profit (Default: Risk-to-Reward Ratio)
extern double        InpRiskRewardRatio    = 1.5;                   // Rasio Risk to Reward (Default: 1 : 1.5 s/d 1 : 2.0)
extern double        InpFixedTPPoints      = 100.0;                 // Target Jarak TP Statis (Points, misal 100 Point = 10 Pips)
...
extern double        InpBreakevenTriggerPips= 5.0;                  // Jarak Profit Memicu BE / SL+ (5.0 Pips = 50 Point)
extern double        InpBreakevenRRTrigger = 1.0;                   // Pemicu BE Saat Profit Mencapai R:R (1.0 = 1:1)
extern double        InpBreakevenLockPips  = 2.0;                   // Pips Keuntungan Terkunci Saat SL+ (2.0 Pips = 20 Point)
extern bool          InpUseTrailingEMA21   = true;                  // Trailing Stop Dinamis Mengikuti EMA 21 Magenta
extern double        InpTrailingBufferPips = 3.0;                   // Jarak Buffer Trailing dari EMA 21 (Pips)

// --- Dynamic Trailing Stop (Kawal Kenaikan Candle & Jarak Points) ---
extern bool          InpUseCandleTrailing  = true;                  // Trailing Naik Mengikuti Low/High Lilin (Candle-by-Candle)
extern double        InpCandleTrailBufferPoints = 15.0;             // Buffer Jarak di Bawah Ekor Lilin (Points)
extern bool          InpUsePointsTrailing  = true;                  // Trailing Stop Dinamis Berdasarkan Jarak Points
extern double        InpTrailingStartPoints= 40.0;                  // Pemicu Trailing Aktif (Setelah Profit 40 Point)
extern double        InpTrailingDistPoints = 30.0;                  // Jarak Pengawalan SL di Belakang Harga (Points)
extern double        InpTrailingStepPoints = 10.0;                  // Langkah Geser SL Tiap Kenaikan Harga (Points)
...
extern double        InpPartialTriggerPips = 6.0;                   // Jarak Profit Pemicu TP1 (6.0 Pips = 60 Point)
...
void CheckTradeSignal()
{
   ...
   if (buyTrendEMA125 && buyDoubleAlign)
   {
      ...
      else
      {
         ...
         if (InpUseDivergenceFilter && CheckRSIDivergence(true))
         {
   ...
   if (sellTrendEMA125 && sellDoubleAlign)
   {
      ...
      else
      {
         ...
         if (InpUseDivergenceFilter && CheckRSIDivergence(false))
         {

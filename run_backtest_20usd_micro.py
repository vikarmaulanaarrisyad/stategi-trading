"""
BACKTEST MIKRO: MODAL $20.00 | LOT 0.01 | XAUUSD M5 (JANUARI 2026 - SEKARANG)
Menguji ketahanan akun modal kecil ($20) dengan fixed lot 0.01 pada 50,123 Bar M5.
Mengevaluasi:
1. Pertumbuhan Saldo dari $20
2. Drawdown Dolar dan Drawdown Persen
3. Margin Requirement & Rekomendasi Leverage (1:500 / 1:1000 / Akun Cent)
4. Performa 4 Preset
"""
...
def simulate_micro(rates_clean, preset_name="1YEAR_OPTIMAL"):
    ...
    for i in range(150, total_bars):
        ...
        if active_trade is not None:
            ...
            # 0.01 lot PnL calculation:
            # (price_diff) * 1.0 = dollar profit on 0.01 lot (since 0.10 lot = $10 per $1 move, 0.01 lot = $1.00 per $1 move)
            lot_mult = t['remaining_lot'] / 0.01  # 1.0 for 0.01 lot
            dollar_per_dollar_move = 1.0

            if t['type'] == 'BUY':
                floating_pnl = t['realized_profit'] + ((c_close - t['open_price']) * lot_mult * dollar_per_dollar_move)
                profit_pips = (c_close - t['open_price']) / pip_value

                cur_eq = balance + floating_pnl
                if cur_eq < min_equity: min_equity = cur_eq
                if cur_eq <= 0.50:  # Margin Call / Stopout
                    blown = True
                    break
                ...
                # Partial TP1 (di 0.01 lot tidak bisa di-split 50% di broker biasa, jadi biarkan jalan penuh ke trailing/TP)
                # Trailing Dynamic
                ...
                if profit_pips >= 5.0 and c_close < ema21[i] and closes[i - 1] >= ema21[i - 1]:
                    exit_price = c_close
                    pnl = t['realized_profit'] + ((exit_price - t['open_price']) * lot_mult * dollar_per_dollar_move)
                    balance += pnl
                    trades.append({'pnl': pnl, 'type': 'BUY', 'reason': 'AUTO-CUT REVERSAL'})
                ...
                if c_low <= t['sl']:
                    ...
                    trades.append({'pnl': pnl, 'type': 'BUY', 'reason': reason})
                    monthly_data[month_key]['trades'] += 1
                    monthly_data[month_key]['pnl'] += pnl
                    if pnl > 0: monthly_data[month_key]['wins'] += 1
                    else: monthly_data[month_key]['losses'] += 1
                ...
                if c_high >= t['tp']:
                    ...
                    pnl = t['realized_profit'] + ((exit_price - t['open_price']) * lot_mult * dollar_per_dollar_move)
                    balance += pnl
                    trades.append({'pnl': pnl, 'type': 'BUY', 'reason': 'TAKE PROFIT (R:R)'})
                    monthly_data[month_key]['trades'] += 1
                    monthly_data[month_key]['pnl'] += pnl
                    if pnl > 0: monthly_data[month_key]['wins'] += 1
                    else: monthly_data[month_key]['losses'] += 1
                ...
                floating_pnl = t['realized_profit'] + ((t['open_price'] - c_close) * lot_mult * dollar_per_dollar_move)
                ...
                if profit_pips >= 5.0 and c_close > ema21[i] and closes[i - 1] <= ema21[i - 1]:
                    exit_price = c_close
                    pnl = t['realized_profit'] + ((t['open_price'] - exit_price) * lot_mult * dollar_per_dollar_move)
                    balance += pnl
                    trades.append({'pnl': pnl, 'type': 'SELL', 'reason': 'AUTO-CUT REVERSAL'})
                    monthly_data[month_key]['trades'] += 1
                    monthly_data[month_key]['pnl'] += pnl
                    if pnl > 0: monthly_data[month_key]['wins'] += 1
                    else: monthly_data[month_key]['losses'] += 1
                ...
                if c_high >= t['sl']:
                    ...
                    balance += pnl
                    reason = 'SL+ LOCK WIN' if exit_price < t['open_price'] else 'STOP LOSS'
                    trades.append({'pnl': pnl, 'type': 'SELL', 'reason': reason})
                    monthly_data[month_key]['trades'] += 1
                    monthly_data[month_key]['pnl'] += pnl
                    if pnl > 0: monthly_data[month_key]['wins'] += 1
                    else: monthly_data[month_key]['losses'] += 1
                ...
                if c_low <= t['tp']:
                    ...
                    pnl = t['realized_profit'] + ((t['open_price'] - exit_price) * lot_mult * dollar_per_dollar_move)
                    balance += pnl
                    trades.append({'pnl': pnl, 'type': 'SELL', 'reason': 'TAKE PROFIT (R:R)'})
                    monthly_data[month_key]['trades'] += 1
                    monthly_data[month_key]['pnl'] += pnl
                    if pnl > 0: monthly_data[month_key]['wins'] += 1
                    else: monthly_data[month_key]['losses'] += 1
        ...
        if buy_trend_125 or is_bull_trap:
            ...
            if (is_pullback and candle_ok and score >= min_confluence) or is_bull_trap:
                ...
                active_trade = {
                    ...
                    'sl': sl_p, 'tp': tp_p, 'remaining_lot': 0.01,
    ...
    return {
        ...
        'max_dd_pct': max_dd_pct, 'final_balance': balance, 'min_equity': min_equity,
        'blown': blown, 'monthly': monthly_data
    }
...
def main():
    ...
    print("=========================================================================================================")
    print("HASIL BACKTEST MODAL $20.00 DENGAN LOT 0.01: JANUARI 2026 - SEKARANG (50,123 BAR M5)")
    print("=========================================================================================================")
    print(f"{'Nama Preset':<28} | {'Trades':<7} | {'Win Rate':<9} | {'PF':<5} | {'Net Profit ($)':<14} | {'Pertumbuhan':<11} | {'Max DD ($)':<10} | {'Status Akun'}")
    print("-" * 115)

    presets = [
        ("1YEAR_OPTIMAL (Default)", "1YEAR_OPTIMAL"),
        ("FAST_AUTO_TRADE", "FAST"),
        ("HIGH_WINRATE_SNIPER", "SNIPER"),
        ("M5_SCALPING_CONFLUENCE", "SCALPING")
    ]

    all_res = {}
    for label, pid in presets:
        res = simulate_micro(rates_clean, pid)
        all_res[pid] = res
        growth = (res['net_profit'] / 20.0) * 100.0
        status = "MC / TUMBANG" if res['blown'] else "SELAMAT (PROFIT)"
        print(f"{label:<28} | {res['total']:<7} | {res['win_rate']:>6.2f}%   | {res['profit_factor']:>4.2f} | ${res['net_profit']:>12,.2f} | {growth:>9.1f}% | ${res['max_dd_dollars']:>8,.2f} | {status}")

    print("=========================================================================================================\n")

    # Detail Bulanan Preset 1YEAR_OPTIMAL
    best = all_res['1YEAR_OPTIMAL']
    print("--- RINCIAN PERFORMA BULANAN PRESET JUARA (1YEAR_OPTIMAL) DENGAN MODAL $20.00 ---")
    print(f"{'Bulan':<10} | {'Trades':<8} | {'Win Rate':<10} | {'Profit Bersih ($)':<18} | {'Status'}")
    print("-" * 65)
    for m, d in best['monthly'].items():
        wr = (d['wins'] / d['trades'] * 100.0) if d['trades'] > 0 else 0
        st = "PROFIT HIJAU" if d['pnl'] > 0 else "LOSS MERAH"
        print(f"{m:<10} | {d['trades']:<8} | {wr:>6.1f}%    | ${d['pnl']:>15,.2f}  | {st}")
    print("-" * 65)
    print(f"Saldo Awal: $20.00 --> Saldo Akhir: ${best['final_balance']:,.2f} (Pertumbuhan: +{(best['net_profit']/20.0)*100:.1f}%)")
    print(f"Titik Ekuitas Terendah (Lowest Equity): ${best['min_equity']:.2f}")
